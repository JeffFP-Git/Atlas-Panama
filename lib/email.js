 ...
 * @param {string} [options.previousCheckDate] - "YYYY-MM-DD" date of the last check compared against, for the "no changes since {date}" line
 */
export async function sendMonitoringUpdateEmail({ recipientEmail, displayName, hasChanges, isFirstRun, changeLines = [], pdfPath, error, language, previousCheckDate }) {
  if (!recipientEmail) {
    console.log('ℹ️  No recipient email for monitoring update; skipping.');
    return false;
  }

  const { t, formatDateForLang } = await import('./emailTranslations.js');
  ...
  if (error) {
    ...
    const previousDate = formatDateForLang(previousCheckDate, lang);
    text = t(lang, 'monitor.noChangesText', { displayName, previousDate });
    html = t(lang, 'monitor.noChangesHtml', { displayName, previousDate });
  }

  if (!error) {
    const subscribeUrl = `${process.env.API_BASE_URL || process.env.FRONTEND_URL || 'http://localhost:3000'}/subscribe`;
    text += '\n' + t(lang, 'monitor.referralText', { subscribeUrl });
    html += t(lang, 'monitor.referralHtml', { subscribeUrl });
  }
...
/**
 * Send one combined update covering every property/entity a subscriber monitors,
 * instead of a separate email per subscription — see lib/dailyScheduler.js's
 * runDailyDigestDispatch, which groups subscriptions by recipient email and calls
 * this once per recipient per dispatch. Works fine for a single item too (the
 * common case today), reading the same as an individual update would.
 * @param {Object} options
 * @param {string} options.recipientEmail
 * @param {'es'|'en'} [options.language]
 * @param {Array<{displayName:string, hasChanges:boolean, isFirstRun:boolean, changeLines:string[], pdfPath:string, previousCheckDate:string}>} options.items
 */
export async function sendDigestEmail({ recipientEmail, language, items }) {
  if (!recipientEmail) {
    console.log('ℹ️  No recipient email for digest email; skipping.');
    return false;
  }
  if (!items || items.length === 0) {
    console.log('ℹ️  No items for digest email; skipping.');
    return false;
  }
  ...
  const changedCount = items.filter(i => i.hasChanges).length;

  const subject = changedCount > 0
    ? t(lang, 'digest.subjectSomeChanges', { changedCount, count })
    : t(lang, 'digest.subjectAllClear', { count });
  const introText = changedCount > 0
    ? t(lang, 'digest.introSomeChanges', { changedCount, count })
    : t(lang, 'digest.introAllClear', { count });
  const introHtml = `<p>${introText}</p>`;

  const itemTextBlocks = [];
  const itemHtmlBlocks = [];
  for (const item of items) {
    if (item.hasChanges) {
      const bulletsText = item.changeLines.map(l => `    • ${l}`).join('\n');
      const bulletsHtml = item.changeLines.map(l => `<li>${l}</li>`).join('');
      itemTextBlocks.push(`${t(lang, 'digest.itemChangeHeader', { displayName: item.displayName })}\n${bulletsText}`);
      itemHtmlBlocks.push(`<p><strong>${t(lang, 'digest.itemChangeHeader', { displayName: item.displayName })}</strong></p><ul>${bulletsHtml}</ul>`);
    } else if (item.isFirstRun) {
      itemTextBlocks.push(t(lang, 'digest.itemFirstRun', { displayName: item.displayName }));
      itemHtmlBlocks.push(`<p>${t(lang, 'digest.itemFirstRun', { displayName: item.displayName })}</p>`);
    } else {
      const previousDate = formatDateForLang(item.previousCheckDate, lang);
      itemTextBlocks.push(t(lang, 'digest.itemNoChange', { displayName: item.displayName, previousDate }));
      itemHtmlBlocks.push(`<p>${t(lang, 'digest.itemNoChange', { displayName: item.displayName, previousDate })}</p>`);
    }
  }

  const disclaimerText = changedCount > 0 ? t(lang, 'monitor.disclaimerText') : '';
  const disclaimerHtml = changedCount > 0 ? t(lang, 'monitor.disclaimerHtml') : '';

  // Attach the unchanged-item PDF too when there are only 1-2 items (small enough that
  // the reassurance-PDF-on-every-check habit still makes sense); at 3+, only attach for
  // items that actually changed or are a first-ever check, and say so explicitly in the
  // cadence note below — user's explicit call, Sept 2026.
  const attachAllPdfs = count <= 2;

  const subscribeUrl = `${process.env.API_BASE_URL || process.env.FRONTEND_URL || 'http://localhost:3000'}/subscribe`;
  const cadenceNote = t(lang, 'digest.cadenceNote', { frequency: t(lang, 'digest.frequencyWeekly') })
    + (attachAllPdfs ? '' : t(lang, 'digest.noAttachmentNote'));

  const text = [introText, '', ...itemTextBlocks, '', disclaimerText, '', cadenceNote, t(lang, 'monitor.referralText', { subscribeUrl })]
    .filter(Boolean).join('\n');
  const html = [introHtml, ...itemHtmlBlocks, disclaimerHtml, `<p style="color:#777;font-size:12px;">${cadenceNote}</p>`, t(lang, 'monitor.referralHtml', { subscribeUrl })]
    .filter(Boolean).join('');

  const fs = await import('fs');
  const attachments = items
    .filter(i => (attachAllPdfs || i.hasChanges || i.isFirstRun) && i.pdfPath && fs.existsSync(i.pdfPath))
    .map(i => i.pdfPath);
...
/**
 * Send the one-time welcome email when a subscription first activates — separate
 * from the daily monitoring emails (sendMonitoringUpdateEmail), which start the
 * next day. Not attached to the change-detection pipeline at all.
 * @param {Object} options
 * @param {string} options.recipientEmail
 * @param {string} options.displayName
 * @param {'es'|'en'} [options.language]
 * @param {string} [options.planLabel] - e.g. "$1/month", from lib/stripe.js's PRICING — omitted from the email if not given
 * @param {string} [options.renewalDate] - "YYYY-MM-DD", the subscription's next renewal — omitted if not given
 */
export async function sendWelcomeEmail({ recipientEmail, displayName, language, planLabel, renewalDate }) {
  if (!recipientEmail) {
    console.log('ℹ️  No recipient email for welcome email; skipping.');
    return false;
  }

  const { t, formatDateForLang } = await import('./emailTranslations.js');
  const lang = language === 'en' ? 'en' : 'es';
  const baseUrl = process.env.API_BASE_URL || process.env.FRONTEND_URL || 'http://localhost:3000';
  const subscribeUrl = `${baseUrl}/subscribe`;
  const tosUrl = `${baseUrl}/terminos`;

  const paymentDetailsText = planLabel
    ? t(lang, 'welcome.paymentDetailsText', { planLabel, renewalDate: renewalDate ? formatDateForLang(renewalDate, lang) : '—' })
    : '';
  const paymentDetailsHtml = planLabel
    ? t(lang, 'welcome.paymentDetailsHtml', { planLabel, renewalDate: renewalDate ? formatDateForLang(renewalDate, lang) : '—' })
    : '';

  const subject = t(lang, 'welcome.subject', { displayName });
  const text = t(lang, 'welcome.text', { displayName, subscribeUrl, tosUrl, paymentDetails: paymentDetailsText });
  const html = t(lang, 'welcome.html', { displayName, subscribeUrl, tosUrl, paymentDetailsHtml });
