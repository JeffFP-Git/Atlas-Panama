 ...
 * @param {'es'|'en'} [options.language]
 */
export async function sendWelcomeEmail({ recipientEmail, displayName, language }) {
  if (!recipientEmail) {
  ...
  const text = t(lang, 'welcome.text', { displayName, subscribeUrl, tosUrl });
  const html = t(lang, 'welcome.html', { displayName, subscribeUrl, tosUrl });
