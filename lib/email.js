...
export async function sendDigestEmail({ recipientEmail, language, items }) {
  ...
  // Attach PDFs only for items worth a full record (changed, or a first-ever baseline) —
  // keeps the combined email lean for subscribers with many properties/entities.
  const fs = await import('fs');
  const attachments = items
    .filter(i => (i.hasChanges || i.isFirstRun) && i.pdfPath && fs.existsSync(i.pdfPath))
    .map(i => i.pdfPath);
