 ...
 * @param {{ highlightPrelacionRows?: number[], highlightNote?: string, lang?: 'es'|'en', hasChanges?: boolean, isFirstRun?: boolean, previousCheckDate?: string }} [opts]
 */
function buildReportHtml(record, opts = {}) {
  ...
  // Status line: what changed (if anything), no-changes-since-date, or first-check baseline.
  // The legal disclaimer is deliberately shown ONLY when something actually changed —
  // showing it on every routine "no changes" report would be alarming and misleading.
  let statusHtml = '';
  if (opts.hasChanges && opts.highlightNote) {
    statusHtml = `<p style="margin-top:16px;"><strong>${esc(t(lang, 'pdf.whatChanged'))}</strong> ${esc(opts.highlightNote)}</p>
    <div class="disclaimer">
      <strong>${esc(t(lang, 'pdf.disclaimerImportant'))}</strong> ${esc(t(lang, 'pdf.disclaimerBody'))}
    </div>`;
  } else if (opts.isFirstRun) {
    statusHtml = `<p style="margin-top:16px;">${esc(t(lang, 'pdf.firstCheck'))}</p>`;
  } else if (opts.previousCheckDate) {
    statusHtml = `<p style="margin-top:16px;">${esc(t(lang, 'pdf.noChangesSince', { previousDate: formatDateForLang(opts.previousCheckDate, lang) }))}</p>`;
  }
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>${REPORT_CSS}</style></head><body>
    ...
    ${Object.entries(tabs || {}).map(([tabName, rows]) => `
    <h2>${esc(tabName)}</h2>
    ${renderTable(null, rows, { lang })}`).join('')}
