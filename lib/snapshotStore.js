...
/** Returns { dateStr, record } for the most recent snapshot strictly before beforeDateStr, or null. */
export function getSnapshotBefore(subscriptionId, beforeDateStr) {
  const dir = subscriptionDir(subscriptionId);
  if (!fs.existsSync(dir)) return null;
  const dateStrs = fs.readdirSync(dir)
    .filter(f => f.endsWith('.json'))
    .map(f => f.slice(0, -5))
    .filter(d => /^\d{4}-\d{2}-\d{2}$/.test(d) && d < beforeDateStr)
    .sort();
  if (dateStrs.length === 0) return null;
  const latestDate = dateStrs[dateStrs.length - 1];
  try {
...
export default { subscriptionDir, todayDateString, snapshotJsonPath, snapshotPdfPath, getLatestSnapshot, getSnapshotBefore, saveSnapshot };
