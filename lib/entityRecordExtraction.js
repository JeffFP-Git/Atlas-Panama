...
import { extractAllOtherTabs } from './fullRecordExtraction.js';
...
export async function extractEntityFullRecord(page, { entityType, ruc }) {
  ...
  const folio = summary['FOLIO'] || prelacion.folio || miembros.mercantilFolio || '';
  // Every other tab on the record (Apoderados — powers of attorney — is the one
  // that matters most here, plus Registro Previo/Electrónico Vigentes/No Vigentes)
  // — pulled every run, per the "full record every run" decision (CLAUDE.md, Sept 2026).
  const tabs = await extractAllOtherTabs(page, folio, ['Prelación', 'Miembros Relacionados']);

  await page.keyboard.press('Escape').catch(() => {});

  return { folio, summary, prelacion, miembros, tabs };
}
