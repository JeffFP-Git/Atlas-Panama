 ...
 */
export function createSubscriptionRequest({ tipo, email, nameOrFolio, name, ruc, folio, codigo, ownerName, relationship, whatsappOptIn, whatsappPhone, language }) {
  const data = loadRequests();
  const id = `${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
  ...
  const request = {
    ...
    // Subscriber's stated relationship to the property/entity ('owner', 'interested_party',
    // or 'third_party') — the on-record version of the TOS §3 legitimate-interest declaration.
